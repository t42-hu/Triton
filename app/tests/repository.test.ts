...
test('profile names reject duplicates on create and rename, including concurrent saves and Hungarian case', async () => {
  ...
  assert.equal(results.find(result => result.status === 'fulfilled')?.value, profile.id);
...
test('staged imports publish atomically, preserve field patches and delete orphan overrides', async () => {
  ...
  assert.equal(replaced.lostOverrides, 1); await publishStages([replaced]);
  ...
  const discarded = await stageSource({ ...input, content: content([]) }, anchor, control());
  await assert.rejects(publishStages([discarded]), /üres forrás/);
  await discardStages([discarded]);
...
test('replacing one profile source leaves another profile timetable intact', async () => {
  await saveProfile('Own isolation');
  await saveProfile('Peer isolation');
  const items = await profiles();
  const own = items.find(item => item.name === 'Own isolation')!;
  const peer = items.find(item => item.name === 'Peer isolation')!;
  const source = (profileId: number, title: string) => ({ id: `${profileId}:import`, profileId, format: 'json' as const, content: content([{ ...event, title }]), name: title, fromDate: '2026-09-01', toDate: '2026-12-31', isManual: 0 });
  await publishStages([await stageSource(source(own.id, 'Own'), anchor, control()), await stageSource(source(peer.id, 'Peer'), anchor, control())]);
  await publishStages([await stageSource(source(own.id, 'Updated'), anchor, control())]);
  assert.deepEqual((await visibleEvents(own.id, '2026-09-07', 7, false)).map(item => item.title), ['Updated']);
  assert.deepEqual((await visibleEvents(peer.id, '2026-09-07', 7, false)).map(item => item.title), ['Peer']);
  await deleteProfile(peer.id);

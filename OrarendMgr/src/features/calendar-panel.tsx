...
type Props = { side: 'left' | 'right'; date: string; profileId: number; profileName: string; events: DisplayEvent[]; common: Set<string>; selectEvent: (event: DisplayEvent) => void; onClose?: () => void };
export function CalendarPanel(props: Props) {
  ...
  return <View className="overflow-hidden rounded-2xl border border-border bg-card">
    <View className="gap-2 border-b border-border p-4">
      <View className="flex-row flex-wrap items-center gap-2"><Text className="shrink font-semibold" numberOfLines={1}>{props.profileName}</Text>{props.side === 'left' ? <Badge variant="secondary"><Text>Saját</Text></Badge> : null}{props.onClose ? <Action quiet icon={X} label={`${props.profileName} naptár bezárása`} onPress={props.onClose}>Bezárás</Action> : null}</View>

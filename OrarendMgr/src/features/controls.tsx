...
export function Modal({ title, description, close, children }: { title: string; description: string; close: () => void; children: ReactNode }) {
  ...
  return <Dialog open onOpenChange={open => { if (!open) close(); }}><DialogContent style={{ width: Math.min(width - 32, 576), maxWidth: 576, maxHeight: height - 32 }}>
    <DialogTitle>{title}</DialogTitle><DialogDescription>{description}</DialogDescription>
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView style={{ maxHeight: Math.max(160, height - 230) }} keyboardShouldPersistTaps="handled" contentContainerStyle={{ gap: 16, paddingBottom: 12 }}>{children}</ScrollView>

...
export function ManualDialog({ profileId, close }: { profileId: number; close: () => void }) {
  ...
  async function save() {
    ...
    try {
      ...
      const stage = await stageSource({ id: `${profileId}:manual:${event.id}`, profileId, format: 'json', content: JSON.stringify({ version: 1, events: [event] }), name: title, fromDate: start.slice(0, 10), toDate: weeks === 'once' ? end.slice(0, 10) : until, isManual: 1 }, app.anchor, { signal: new AbortController().signal, progress: () => undefined });
